...
def convert_coco_json(json_dir: str, use_segments=False, cls91to80=False):
    """Converts COCO JSON format to YOLO label format, with options for segments and class mapping."""
    save_dir = "../../data/interim"  # output directory
    coco80 = coco91_to_coco80_class()

    # Import json
    json_labels = [path for path in Path(json_dir).resolve().glob("**/*.json") if path.is_file() and (not path.parent.startswith("__"))]
    for json_file in sorted(json_labels):
        fn = Path(save_dir) / "__full_data"  # folder name
        fn.mkdir()
        with open(json_file) as f:
            data = json.load(f)
